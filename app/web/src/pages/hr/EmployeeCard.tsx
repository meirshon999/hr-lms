...
interface Card {
  id: string; iin: string; full_name: string; phone: string; start_date: string; position: string;
    ...
    <>
      ...
      <div className="panel">
        ...
        {editing
          ...
          : (
            <dl className="kv">
              <dt>ИИН</dt><dd style={{ fontFamily: 'monospace' }}>{data.iin}</dd>
              <dt>Точка</dt>
              <dd>
                {data.location}
                {data.stage !== 'archived' && (
                  <button className="btn ghost sm" style={{ padding: '2px 10px', marginLeft: 8 }}
                    onClick={() => setTransfer(true)}>Перевести</button>
                )}
              </dd>
      ...
      {transfer && (
        <TransferDialog
          ...
          onDone={(msg) => { setTransfer(false); toast(msg); bump(); reload(); }}
        />
      )}
 ...
 *
 * Показываем заранее, чем это обернётся для человека: часть уроков на новой
 * точке своя, и их придётся пройти заново. HR должен понимать это до нажатия,
 * а не узнавать из карточки после.
 */
function TransferDialog(
  { data, onClose, onDone }: { data: Card; onClose: () => void; onDone: (msg: string) => void },
) {
  const { data: locs } = useAsync(() => get<{ items: { id: string; name: string }[] }>('/locations'), []);
  const [to, setTo] = useState('');
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const options = (locs?.items ?? []).filter((l) => l.id !== data.location_id);

  async function submit() {
    if (!to) return;
    setBusy(true); setErr(null);
    try {
      const r = await post<{ kept: number; redo: number; added: number }>(
        `/employees/${data.id}/transfer`, { location_id: to });
      const parts = [
        r.kept ? `зачтено уроков: ${r.kept}` : '',
        r.redo ? `заново: ${r.redo}` : '',
        r.added ? `добавлено: ${r.added}` : '',
      ].filter(Boolean);
      onDone('Сотрудник переведён' + (parts.length ? ` — ${parts.join(', ')}` : ''));
    } catch (e) {
      setErr(e instanceof ApiError ? e.message : 'Не удалось перевести');
      setBusy(false);
    }
  }
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <h3>Перевод на другую точку</h3>
        <p className="muted" style={{ fontSize: 13, marginBottom: 14 }}>
          Сейчас: <b>{data.location}</b>. Общие уроки останутся сданными, а уроки, которые
          на каждой точке свои, откроются заново — под новую точку.
        </p>
        <label className="field"><span>Новая точка</span>
        ...
        <div className="row">
          ...
          <button className="btn sm" disabled={busy || !to} onClick={submit}>
