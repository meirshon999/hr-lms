...
interface Me {
  user: { id: string; login: string; role: Role; must_change_password?: boolean };
  employee: { id: string; full_name: string; stage: string; position_id: string; location_id: string } | null;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(login: string, password: string): Promise<Role> {
    const r = await post<{ token: string; role: Role; must_change_password: boolean }>(
