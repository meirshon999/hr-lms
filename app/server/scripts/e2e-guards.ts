...
async function hire(pid: string, login: string, name: string, phone: string) {
  const r = await j('/employees', {
    method: 'POST', h: hrH,
    body: {
      iin: testIin(Date.now() % 7000 + (hireSeq += 13)),
      full_name: name, position_id: pid, location_id: anyLocation, phone,
      start_date: new Date().toISOString().slice(0, 10), login, password: login + 'pass1',
    },
  });
...
async function passPre(login: string) {
  const tok = (await j('/auth/login', { method: 'POST', body: { login, password: login + 'pass1' } })).d.token;
...
async function main() {
  ...
  anyLocation = (await j('/locations', { h: hrH })).d.items[0].id;
