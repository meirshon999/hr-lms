  ...
  try {
    response = await client.messages.create({
      ...
      system:
        ...
        + 'пояснений. Ничего не придумывай и ничего не пропускай.\n'
        // Разделы — не украшение: по ним потом строится дерево блоков и уроков.
        // Без пометок документ приезжает сплошным текстом, и структура,
        // которая в нём видна глазами, теряется на ровном месте.
        + 'Заголовок раздела начинай с «## », подзаголовок — с «### ». '
        + 'Если в документе есть нумерация разделов, сохраняй её в заголовке.\n'
        + 'Таблицы переноси строками, значения через « | ».',
      messages: [{
        role: 'user',
        content: [
          {
