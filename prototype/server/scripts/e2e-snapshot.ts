...
async function main() {
  ...
  const emp = (await j('/employees', {
    method: 'POST', h,
    body: { full_name: 'Снимок Тестов', position_id: pos.id, phone: '+7999' + sfx + '1', start_date: new Date().toISOString().slice(0, 10), login: 'snap' + sfx, password: 'snappass1' },
  }));
  ...
  const problems = (await j(`/trajectories/${pos.id}`, { h })).d.problems;
  check('живая траектория теперь неполная', problems.length > 0);
