...
export interface AuthUser { id: string; login: string; role: Role; employee_id: string | null; }
...
export function loadUser(req: FastifyRequest): AuthUser | null {
  ...
  if (!t) return null;
  const u = one<any>('SELECT id, login, role, employee_id, is_active FROM users WHERE id = ?', t.uid);
  if (!u || !u.is_active) return null;
  return { id: u.id, login: u.login, role: u.role, employee_id: u.employee_id };
}
